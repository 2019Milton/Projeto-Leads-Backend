...
function contextoNicho(lead: any) {
  ...
  if (isTelecom) return {
    nicho: "Telecom Empresarial",
    produto: "solução de telecom/TI",
    produto_pl: "soluções de telecom/TI",
    verbo_interesse: "contratar uma solução de telecom",
    qualificadores: ["tipo de serviço", "porte da empresa", "situação atual (primeira contratação ou troca)", "região atendida"],
    perguntas: [
      "Qual serviço você tem interesse: internet dedicada, firewall, PABX, hotspot ou SmartChat?",
      "Qual o porte da sua empresa (número de funcionários)?",
      "Você já tem um fornecedor hoje ou seria a primeira contratação?",
      "Qual a cidade ou região da sua empresa?",
      "Tem alguma urgência para resolver isso?"
    ],
    msg_quente: (nome: string) => `Oi ${nome}! Vi seu interesse em telecom/TI empresarial. Qual o melhor horário para eu te ligar hoje e entender sua necessidade?`,
    msg_morno:  (nome: string) => `Oi ${nome}! Para te indicar a melhor solução, me conta: qual serviço te interessa e qual o porte da sua empresa?`,
    msg_frio:   (nome: string) => `Oi ${nome}! Você ainda tem interesse em melhorar a estrutura de telecom da sua empresa? Posso te enviar mais informações sem compromisso.`,
    msg_rec_alta: (nome: string) => `Oi ${nome}! Vi que você tinha interesse em soluções de telecom. Ainda faz sentido? Temos condições especiais essa semana.`,
...
await client.query(`
  CREATE TABLE IF NOT EXISTS campanhas_telecom (
    campanha_id     INTEGER PRIMARY KEY REFERENCES campanhas(id) ON DELETE CASCADE,
    tipo_servico    TEXT,
    porte_empresa   TEXT,
    situacao_atual  TEXT,
...
async function copiarDadosNichoCampanha(
  ...
  if (nichoSlug === "imoveis") {
    ...
    const r = await dbClient.query(
      `SELECT tipo_servico, porte_empresa, situacao_atual, area_atendida
       FROM campanhas_telecom WHERE campanha_id = $1`,
      [origCampanhaId]
    );
    if (r.rows.length > 0) {
      const d = r.rows[0];
  ...
  try {
    ...
    const campanhas = await client.query(
      ...
      SELECT
        ...
        ctc.tipo_servico AS tipo_servico_telecom,
        ctc.porte_empresa,
        ctc.situacao_atual,
...
app.post("/ia/campanhas/criador", authMiddleware, async (c) => {
  ...
  const nichoConfig: Record<string, {
    ...
    telecom: {
      topicoDefault: "solucao de telecom empresarial",
      especialidade: "telecomunicacoes e TI empresarial (internet dedicada, firewall, PABX, hotspot, chatbot) no Brasil",
      v1exemplos: [
        "Sua internet caiu de novo essa semana?",
        "Quantas ligacoes voce perdeu hoje por falta de estrutura?",
        "Ate quando sua empresa vai operar sem seguranca de rede?"
      ],
      v1texto: "crie urgencia em torno do risco e prejuizo de continuar sem a solucao (queda de internet, ligacoes perdidas, vulnerabilidade a ataques)",
      v2exemplos: [
        "Sua empresa merece uma estrutura que nao falha",
        "Tenha a tranquilidade de uma rede em que voce pode confiar",
        "Foque no seu negocio, a gente cuida da infraestrutura"
      ],
      v2texto: "evoque tranquilidade, confianca profissional e a sensacao de estar em boas maos",
      v3exemplos: [
        "Reduza custos em ate 70% com telefonia em nuvem",
        "99,97% de uptime garantido em contrato",
        "Implementacao em 24h, sem investimento inicial"
      ],
      v3texto: "destaque numeros concretos: reducao de custo, uptime garantido, prazo de implementacao, ROI",
      perguntas: "Qual servico voce tem interesse: internet dedicada, firewall, PABX, hotspot ou SmartChat?\nVoce ja tem um fornecedor hoje ou seria a primeira contratacao?\nQual o porte da sua empresa?",
...
// ─── ChatIALeads: assistente virtual da plataforma ───────────────────────────
app.post("/ia/chat-leads", authMiddleware, async (c) => {
  try {
    const user: any = c.get("user");
...
## SEÇÕES (menu lateral esquerdo)

### Dashboard
Visão geral de resultados: total de leads, campanhas ativas/pausadas, investimento total, CPL (Custo por Lead), CTR (Taxa de Clique), alcance, impressões. Gráficos de performance diária. Saúde das campanhas com avaliação automática. Apenas visualização — sem ações de edição.

### Plataformas (Integrações)
Gerencia conexões com plataformas de anúncio. Atualmente disponível: Meta Ads. Em breve: Google Ads, TikTok Ads, LinkedIn Ads, Kwai Ads, Pinterest Ads, Snapchat Ads, Microsoft Ads.
Dentro do card META ADS:
- Botão "Conectar": inicia autorização OAuth do Facebook — o usuário faz login na sua conta Meta/Facebook
- Após conectar: seletor de PÁGINA DO FACEBOOK (qual página aparece como anunciante nos anúncios)
- Seletor de CONTA DE ANÚNCIOS (qual conta Meta vai pagar os anúncios — precisa ter saldo ou cartão cadastrado NA META, não na plataforma)
- Seletor de INSTAGRAM vinculado (conta Instagram que aparece nos anúncios do feed do Instagram)
- Seção PAGAMENTO: link para gerenciar pagamento diretamente na Meta (abre business.facebook.com)
- Botão "Verificar conexão": testa se tudo está configurado corretamente
- Status "Conectado" em verde = pronto para criar e publicar campanhas
Como adicionar saldo na Meta: acesse business.facebook.com → Configurações de pagamento → adicione cartão de crédito. O saldo é gerenciado pela Meta, não pela plataforma.

### Campanhas
Duas áreas principais:

**CRIAR CAMPANHA** (botão "Criar Campanha" no topo da página):
Seção "📋 Configuração da campanha":
  - Botões de plataforma: Meta Ads (único disponível), Google/TikTok/LinkedIn/outros (em breve, desabilitados)
  - Campo "Nome da campanha": nome interno, só você vê — use algo descritivo como "Plano Familiar SP - Amil"
  - Seletor "Página Facebook": qual página aparece como anunciante — precisa já ter conectado em Plataformas
  - Checkboxes "Onde anunciar": Facebook e Instagram (ambos marcados por padrão)

Seção "🖼️ Imagens e texto":
  - Botão "Adicionar imagem": upload de até 10 imagens JPG/PNG (quando há 2+, vira carrossel automaticamente)
  - Botão "Enviar vídeo": upload MP4/MOV/WebM — substitui imagens quando usado
  - Botão "🤖 Gerar Banner com IA": cria imagem automaticamente com IA baseada no nicho e texto
  - Botão "✏️ Editar imagem": editar uma imagem existente com IA
  - Campo "Texto do anúncio": texto principal que aparece no feed — escreva a proposta de valor clara
  - Seletor "Botão do anúncio (CTA)": Saiba mais (padrão) / Cadastre-se / Solicitar agora

Seção "👥 Público e orçamento":
  - "Orçamento diário (R$)": valor mínimo recomendado R$30/dia para aprendizado da Meta
  - Campo "País": BR (padrão Brasil)
  - Seletor "Gênero": Todos / Homens / Mulheres
  - Campos "Idade mínima / máxima": de 18 a 65+
  - "Busca de interesses": campo de busca — digite ex: "planos de saúde", selecione e aparecem como chips
  - "Busca de localidades": cidades, estados, CEPs, regiões — aparecem como chips removíveis

Seção "📅 Agendamento e custo":
  - "Data de início": quando a campanha começa (padrão: imediato)
  - "Data de término": opcional — quando a campanha para automaticamente
  - "Controle de custo": Automático da Meta (recomendado), Menor custo, Valor de controle (lance máximo por lead)
  - "Valor de controle": campo que aparece quando selecionado "Valor de controle" — valor em R$ máximo por lead
  - Checkbox "CBO": Campaign Budget Optimization — Meta distribui orçamento automaticamente entre conjuntos
  - "Janela de atribuição": período para contar conversões (padrão da Meta)
  - "Categoria especial": Moradia/Imóveis, Emprego, Crédito/Financeiro, Questões sociais — obrigatório quando o produto se enquadra (risco de rejeição pelo Meta se não configurado)

Seção "🎯 Segmentação avançada":
  - Posições Facebook: Feed, Coluna direita, Stories, Reels, In-stream video, Artigos instantâneos, Marketplace, Busca
  - Posições Instagram: Feed, Stories, Reels, Explore, Shopping
  - Dispositivos: Celular / Desktop (ambos por padrão)
  - Checkbox "Formulário de qualidade": filtra leads menos qualificados (pode reduzir volume)
  - Checkbox "Público Advantage+ Meta": permite à Meta expandir o público automaticamente
  - Seletor "Conta Instagram": conta específica para anúncios no Instagram

Seção de CRIATIVO (título, descrição, link):
  - "Título do anúncio": aparece em destaque na publicação
  - "Descrição": subtítulo do anúncio
  - "Link de destino": URL da landing page ou formulário externo

Seção FORMULÁRIO (varia por nicho):
  - "Política de privacidade (URL)": link obrigatório para o formulário da Meta
  - "Texto da política": mensagem de consentimento exibida no formulário
  - "Perguntas qualificadoras": até 4 perguntas que o lead responde antes de enviar o contato
  - "Título obrigado": título da tela de confirmação após o lead se cadastrar
  - "Mensagem obrigado": texto de confirmação
  - "Botão obrigado": texto do botão (ex: "Ver site")
  - "URL obrigado": página para onde vai após cadastro

Botão "🚀 Publicar Campanha": publica tudo na Meta de uma vez — precisa ter Meta conectada e conta de anúncios com saldo/cartão
Botão "💡 IA Ouro — Preencher com IA": preenche campos automaticamente com IA baseada no nicho

Campos específicos por NICHO (aparecem automaticamente ao selecionar o nicho):
- Imóveis: Tipo de imóvel (residencial/comercial/terreno/rural), Finalidade (comprar/alugar), Valor mínimo (R$), Valor máximo (R$), Região/Bairro
- Planos de Saúde: Operadora, Tipo de plano (individual/familiar/empresarial/por adesão), Faixa etária mínima/máxima, Cobertura (nacional/estadual/municipal), Acomodação (enfermaria/apartamento)
- Suplementos: Produto, Objetivo (ganho de massa/emagrecimento/saúde geral/performance), Marca, Público-alvo
- SaaS: Tipo de produto, Público-alvo

**MINHAS CAMPANHAS** (lista de campanhas já criadas):
Cada card mostra: nome, nicho, status (ACTIVE/PAUSED/etc.), leads captados, orçamento/dia, gasto total, CPL, CTR, alcance, cliques, impressões.
Botões em cada card:
  - ⚙️ Detalhes: abre painel lateral direito com todos os detalhes (IDs Meta, configurações completas, segmentação, criativo)
  - ✏️ Editar: abre modal de edição — mesmos campos da criação, alterações são enviadas diretamente à Meta
  - Duplicar: cria cópia local da campanha para editar e publicar como nova campanha separada
  - Enviar/Encaminhar: envia a campanha para um corretor vinculado publicar pela conta Meta dele
  - Sincronizar: atualiza dados desta campanha com a Meta (status real, métricas atualizadas)
  - Pausar / Ativar (toggle): pausa ou reativa a campanha na Meta imediatamente
  - Excluir: remove a campanha da plataforma (não apaga da Meta)
- Botão "Publicar": aparece em campanhas ainda não publicadas — clique para publicar na Meta
- Filtros: seletor de nicho (Planos de Saúde, Imóveis, etc.) e seletor de status (Todas, Ativas, Pausadas, Rascunho, etc.)
- Botão "Sincronizar" (no topo): sincroniza todas as campanhas de uma vez

### Central de Leads
Lista de todos os leads captados. Filtros: campanha específica, período (hoje/semana/mês/personalizado), status, busca livre por nome/telefone/email.
Kanban de status (arrastar ou clicar para mover): Novo → Em Contato → Fechado / Perdido
Cards de lead: nome, telefone/WhatsApp, email, campanha, data de captação, status atual.
Ao clicar em um lead (ou no botão de detalhes): abre painel lateral com:
  - Dados completos (todas as respostas do formulário)
  - Botão WhatsApp: abre conversa direta via wa.me com o número do lead
  - Histórico de contatos
  - Análise IA: sugere abordagem personalizada baseada no perfil e respostas
  - Botões de status: Marcar em contato / Fechar / Perder
Botão "Recuperação de perdidos" (ou "Recuperação de leads" no ChatIALeads): prioriza leads perdidos com maior chance de retorno e sugere abordagem de recontato.
Resumo do corretor: análise por período (hoje, semana, mês) — mostra prioridades, oportunidades e leads que merecem atenção imediata.

### Suporte
Chat em tempo real com equipe de atendimento humano. Envio de mensagens, prints e arquivos. Histórico de conversas. Para dúvidas técnicas ou problemas na plataforma.

### Planos e Recursos
Seu plano atual, recursos disponíveis, limites (campanhas, leads, IA) e o que está liberado.

### Admin / Painel Suporte (visível apenas para administradores)
- Painel Admin: gestão de usuários, corretores vinculados, configurações
- Painel Suporte: responder chats dos usuários

## REGRAS DE RESPOSTA
- Seja ESPECÍFICO: diga EXATAMENTE onde clicar, qual campo, qual botão — "clique em X" não "acesse a área de X"
- Respostas CURTAS: máx 3-4 frases diretas. Se precisar de mais, use lista com bullets (-)
- Use linguagem informal e prática: "Vai em Plataformas, clique em Conectar no card Meta..."
- Se a resposta direcionar para uma seção navegável, inclua no FINAL: [NAVEGAR:secao:Texto do botão]
  - Valores de secao: dashboard / integracoes / campanhas / leads / suporte / informacoes
  - Exemplo: [NAVEGAR:integracoes:Abrir Plataformas] ou [NAVEGAR:campanhas:Abrir Campanhas]
- Inclua [NAVEGAR:...] apenas quando for útil navegar para a seção — não em respostas genéricas
- Nunca invente funcionalidades que não existam na plataforma
- Não mencione preços em R$ da plataforma — redirecione para Planos e Recursos`;

    const mensagemUsuario = `Mensagem: ${mensagem}\n\nContexto: ${JSON.stringify(contexto || {}, null, 2)}`;

    let resposta: string | null = null;

    // Tenta Anthropic (haiku — rápido e barato para chat)
    const anthropicKey = Bun.env.ANTHROPIC_API_KEY;
    if (anthropicKey) {
      try {
        const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        const data: any = await resp.json();
        if (resp.ok) resposta = data?.content?.[0]?.text?.trim() || null;
      } catch (_) {}
    }
    ...
    if (!resposta) {
      ...
      if (openaiKey) {
        try {
          ...
          const data: any = await resp.json();
          if (resp.ok) resposta = data?.choices?.[0]?.message?.content?.trim() || null;
        } catch (_) {}
      }
    }

    if (!resposta) {
      return c.json({ error: "IA indisponível" }, 503);
    }
    ...
    console.error("ERRO CHAT IA LEADS:", err);
    return c.json({ error: "Erro no chat" }, 500);
  }
});
